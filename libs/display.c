...
static char* _first_line_message = "";
static char* _second_line_message = "";
static char* _alert = "";
...
static bool display_update() {
    ...
    draw_string("Estacao Alerta", 8, 6);
    draw_string(_first_line_message, 10, 16);
    draw_string(_second_line_message, 10, 28);
    draw_string(_alert, 10, 41);
    send_data();
    return true;
}
...
void display_set_rain_volume(unsigned int volume) {
    static char message[32];
    sprintf(message, "Chuva: %d%%", volume);
    _first_line_message = message;
}

void display_set_river_level(unsigned int level) {
    static char message[32];
    sprintf(message, "Rios: %d%%", level);
    _second_line_message = message;
}

void display_set_alert(char* alert) {
    _alert = alert;
}